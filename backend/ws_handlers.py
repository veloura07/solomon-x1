...
-------
  ...
  { "event": "emotion_update","ring_id": "...", "valence": -1.0..1.0, "arousal": 0.0..1.0 }
  { "event": "error",         "message": "..." }
"""
...
async def _handle_user_message(
    ...
    Streams tokens back to the frontend as they arrive, then scores the
    completed response for emotional tone and broadcasts that too.
    """
    ...
    # ── Run generation ────────────────────────────────────────────────────────
    full_response = await ring_engine.process_message(
        ring_id=ring_id,
    ...
    if not session.abort_requested:
        ...
        # Score the completed response for emotional tone and broadcast it.
        # Skipped on abort/failure (full_response is "" in both cases) — no
        # point tinting the avatar based on a response that never finished.
        if full_response:
            emotion = emotion_engine.analyze(ring_id, full_response)
            await _send(websocket, {
